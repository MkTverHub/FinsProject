...
public interface PurposeRepository extends JpaRepository<Purpose, Integer> {
    ...
    //По проекту
    @Query(value = "select id,description,name,par_row_id,ROUND(AVG(expense)"+strSpecSymbol+strSpecSymbol+"numeric,2) as expense,ROUND(AVG(profit)"+strSpecSymbol+strSpecSymbol+"numeric,2) as profit from purpose where par_row_id = :fins_project_id group by id\n", nativeQuery = true)
