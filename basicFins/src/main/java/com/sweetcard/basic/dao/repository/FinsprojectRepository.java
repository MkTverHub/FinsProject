...
public interface FinsprojectRepository extends JpaRepository<Finsproject, Integer> {
    ...
    @Query(value = "select t1.id, t1.name, t1.description, to_char(t1.created, 'yyyy-mm-dd hh24:mi:ss') as created from finsproject t1 where t1.id in (select project_id from it_proj_user where user_id = (select id from app_user where email = :user_login))", nativeQuery = true)
